...
if __name__ == '__main__':
  ...
  parser.add_argument('--user', help='the username for connection', required=True)
  parser.add_argument('--pass', help='the password for connection', required=True)
  parser.add_argument('--group', help='the group to be dropped', required=True)
  args = parser.parse_args()
  args_dict = vars(args)

  conn = psycopg2.connect(f"dbname='{args_dict['db']}' user='{args_dict['user']}' host='{args_dict['host']}' password='{args_dict['pass']}' port={args_dict['port']}")
  cur = conn.cursor()
